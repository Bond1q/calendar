import { AbsenceReducer, AbsenceTypes } from "src/app/types/types";
...
export const absenceReducer = createReducer(initialState,
	on(deleteAbsence, (state, action) => {
		return { ...state, absencePeriods: state.absencePeriods.filter((absence) => absence.id !== action.id) }
	}),
