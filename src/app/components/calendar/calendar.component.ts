...
import { AbsencePeriod, AbsencePeriodInCalendar, DateInfo, AbsenceTypes } from '../../types/types'
...
export class CalendarComponent implements OnInit {
	...
	date = new Date();
	absenceList = list;
	toppings = this._formBuilder.group(this.absenceTypes.reduce((acc: { [key: string]: boolean }, item) => {
		acc[item] = true;
		return acc;
	}, {}));
	...
	setAbsenceDays(dates: Date[]): DateInfo[] {
		return dates.map((date) => {
			const newAbsenceList = this.absenceList.reduce((prev, item) => {
				if (moment(date).isBetween(moment(item.dateStart), moment(item.dateEnd), undefined, '[]') && this.toppings.value[item.type]) {
